...
def unet_generator(output_channels, norm_type='batchnorm'):
    """Modified U-Net generator for 512x512 input and output."""
    ...
    up_stack = [
        ...
        upsample(64, 4, norm_type),  # **NEW: (bs, 512, 512, 64)**
    ]

    initializer = tf.random_normal_initializer(0., 0.02)

    last = tf.keras.layers.Conv2DTranspose(
        output_channels, 4, strides=2,  # **Keep strides=1 for 512x512**
        padding='same', kernel_initializer=initializer,
        activation='tanh')  # **Output (bs, 512, 512, 3)**

    concat = tf.keras.layers.Concatenate()

    inputs = tf.keras.layers.Input(shape=[512, 512, 3])  # **Changed input size**
    x = inputs
...
def discriminator(norm_type='batchnorm', target=True):
    """PatchGAN discriminator for 512x512 images."""
    
    initializer = tf.random_normal_initializer(0., 0.02)

    inp = tf.keras.layers.Input(shape=[512, 512, 3], name='input_image')  # **Changed Input**
    x = inp
    ...
    down4 = downsample(512, 4, norm_type)(down3)  # (bs, 32, 32, 512)
    down5 = downsample(512, 4, norm_type)(down4)  # **NEW: (bs, 16, 16, 512)**
    down6 = downsample(512, 4, norm_type)(down5)  # **NEW: (bs, 8, 8, 512)**

    zero_pad1 = tf.keras.layers.ZeroPadding2D()(down6)  # (bs, 10, 10, 512)
