...
def unet_generator(output_channels, norm_type='batchnorm'):
    """Modified U-Net generator for CycleGAN with higher output resolution."""
    ...
    up_stack = [
        ...
        upsample(64, 4, norm_type),  # **NEW: (bs, 256, 256, 64)**
    ]
    ...
    last = tf.keras.layers.Conv2DTranspose(
        ...
        activation='tanh')  # **NEW: Produces (bs, 512, 512, 3)**

    concat = tf.keras.layers.Concatenate()

    inputs = tf.keras.layers.Input(shape=[None, None, 3])  # Input remains 256x256
    x = inputs
...
def discriminator(norm_type='batchnorm', target=True):
    """Modified PatchGAN discriminator for 512x512 images."""
    ...
    zero_pad1 = tf.keras.layers.ZeroPadding2D()(down5)  # (bs, 18, 18, 512)
